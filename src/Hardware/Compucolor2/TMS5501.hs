...
tms5501 MkInput{..} cmd = (dataOut, out)
  where
    ...
    (rxResult, rxFlags) =
        mealyStateB (uncurryN $ UART.uartRx (SNat @(DomainPeriod dom))) UART.initRxS (serialIn, rxReset)

    (serialOut, txReady) =
        mealyStateB (uncurryN $ UART.uartTx (SNat @(DomainPeriod dom))) UART.initTxS (txNew, txBreak)
